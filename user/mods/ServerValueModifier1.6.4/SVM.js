...
class MainSVM
{
	preAkiLoad(container)
	{
		...
		{	
				...
				Logger.warning("There is an error that will help you distinguish what went wrong:\n");
		...
		//HEALTH FUNCTIONS
		if (Config.Player.EnableHealth || Config.Scav.EnableScavHealth || Config.Scav.ScavCustomPockets) //TO OVERRIDE HEALTH + CURRENT SCAV HEALTH AND POCKETS BEFORE RAID
		{
	 ...
	 postDBLoad(container)
	{
		...
		Logger.log(`SVM 1.6.4 has initialized, I left a cool comment in other pants`, "blue");
		...
		if (Config.Bots.EnableBots)
		{
			...
			Bots.maxBotCap.factory = Config.Bots.BotSpawn.FactoryLimit;
		...
		if (Config.Items.EnableItems)
		{
			...
			for (const id in items)
			{
				let base = items[id] //I'm consistent af, need to either ditch such usage or switch to it, smh.
		...
		if (Config.Traders.EnableTraders)
		{
			...
			trader.fence.discountOptions.assortSize = 0;
			...
			for (let durabthreshold in trader.durabilityPurchaseThreshhold)
			if(durabthreshold !== "579dc571d53a0658a154fbec")
			{
				trader.durabilityPurchaseThreshhold[durabthreshold] =  Config.Traders.MinDurabSell
			}
			globals.TradingSettings.BuyoutRestrictions.MinDurability =  Config.Traders.MinDurabSell / 100
			//trader.durabilityPurchaseThreshhold = Config.Traders.MinDurabSell
